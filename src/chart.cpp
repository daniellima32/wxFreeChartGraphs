...
void Chart::Draw(ChartDC &cdc, wxRect &rc, bool antialias)
{
    ...
    if (m_header && !m_header->IsEmpty()) {
        ...
        m_header->Draw(cdc.GetDC(), headerRect); //Desenha o título
    }
	...
	m_plot->Draw(cdc, rc, PLOT_DRAW_BACKGROUND); //Desenha o fundo .. borda, valores de borda
    
    m_plot->Draw(cdc, rc, PLOT_DRAW_DATA); //desenha os dados do gráfico
}
