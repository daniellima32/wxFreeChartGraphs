...
wxRect BarType::getRcPlotBackup()
{
	return rcPlotBackup;
}
void BarType::setRcPlotBackup(wxRect rect)
{
	rcPlotBackup = wxRect(rect);
}

void BarType::setOriented(bool par)
{
	oriented = par;
}
        ...
        bool vertical, size_t item, CategoryDataset *dataset)
{
    FOREACH_SERIE(serie, dataset) {
		...
		if (oriented)
		{
			//xBaseG = horizontalMirroring(xBaseG); //Não precisa espelhar na horizontal
			yBaseG = verticalMirroring(yBaseG);
...
wxRect BarRenderer::getRcPlotBackup()
{
	return rcPlotBackup;
}
void BarRenderer::setRcPlotBackup(wxRect rect)
{
	rcPlotBackup = wxRect(rect);
}

BarRenderer::BarRenderer(BarType *barType, bool oriented)
{
    m_barType = barType;
...
void BarRenderer::Draw(wxDC &dc, wxRect rc, Axis *horizAxis, Axis *vertAxis, bool vertical, CategoryDataset *dataset)
{
    for (size_t n = 0; n < dataset->GetCount(); n++) {
        //configura o rect
		m_barType->setRcPlotBackup(rcPlotBackup);
		m_barType->setOriented(oriented);
