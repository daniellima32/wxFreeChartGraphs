...
NumberAxis::NumberAxis(AXIS_LOCATION location, bool oriented)
: LabelAxis(location, oriented)
{
    // default values
    m_tickFormat = wxT("%.2f");
...
{
    return m_minValue + step * m_labelInterval; //original
}*/

double NumberAxis::GetValue(size_t step)
{
	return m_minValue + step * m_labelInterval; //original
	//return m_maxValue - step * m_labelInterval;
	//return 0;
}
