...
Axis::Axis(AXIS_LOCATION location, bool oriented)
{
    m_location = location;
...
wxCoord Axis::ToGraphics(wxDC& WXUNUSED(dc), int minCoord, int gRange, double value)
{
	...
	//double maxCoord = minCoord + m_marginMin; //new
    ...
    if (m_useWin) {
        minValue = m_winPos; //original
        maxValue = m_winPos + m_winWidth;
    }

    return ::ToGraphics(minCoord, gRange, minValue, maxValue, 0/*textMargin*/, IsVertical(), value); //origin
	//return ::ToGraphics(maxCoord, gRange, minValue, maxValue, 0/*textMargin*/, IsVertical(), value);
}
...
wxRect Axis::getRcPlotBackup()
{
	return rcPlotBackup;
}
void Axis::setRcPlotBackup(wxRect rect)
{
...
wxCoord ToGraphics(int minCoord, int gRange, double minValue, double maxValue, wxCoord margin, bool vertical, double value)
{
    ...
    if (vertical) {
        k = (maxValue - value) / valueRange; //original
		//k = (value + minValue) / valueRange;
    }
    else {
        k = (value - minValue) / valueRange; //original
		//k = (maxValue + value) / valueRange; //deve alterar para decrementar a sequencia
    }

    //return (wxCoord) (k * gRange + minCoord); //origin
	return (wxCoord)(k * gRange + minCoord);  //deve ser a máxima coord menos o desl
}
