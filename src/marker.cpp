...
int Marker::horizontalMirroring(int value)
{
	return mirroring(rcPlotBackup.x, rcPlotBackup.x + rcPlotBackup.width, value);
}

int Marker::verticalMirroring(int value)
{
	return mirroring(rcPlotBackup.y, rcPlotBackup.y + rcPlotBackup.height, value);
}

int Marker::mirroring(int min, int max, int value)
{
	int interval = value - min;
	return max - interval;
}

wxRect Marker::getRcPlotBackup()
{
	return rcPlotBackup;
}
void Marker::setRcPlotBackup(wxRect rect)
{
	rcPlotBackup = wxRect(rect);
}

void Marker::setOriented(bool par)
{
	oriented = par;
}
...
void LineMarker::Draw(wxDC &dc, wxRect rcData, Axis *horizAxis, Axis *vertAxis)
{
	...
	if (oriented)
	{
		y0 = verticalMirroring(y0); //espelhamento vertical
		y1 = verticalMirroring(y1);  //espelhamento vertical
	}
